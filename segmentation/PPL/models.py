...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x, mem):
        ...
        return x
class ProbabilisticDecoderLayer(nn.Module):
    def __init__(
        self,
        d_model,
        nhead,
    ...
    def forward(self, x, mem):
        ...
        x = x + self.self_attn(q, k, v)
        x = q = self.norm2(x)
        x = self.mlp2(x) + self.dropout(self.mlp(self.cross_attn(q, mem, mem)))
        return x
...
class VCPDecoder(nn.Module):
    def __init__(self,
        ...
        self.decoder = nn.ModuleList([
                    ProbabilisticDecoderLayer(transformer_width, transformer_heads, dropout) for _ in range(transformer_layers)
                ])
        # self.probdecoder = ProbabilisticDecoderLayer(transformer_width, transformer_heads, dropout)
